...
// PUT /api/pedidos/:pedido/itens/substituir/:nome_antigo
// Substitui um item existente por um novo, usando o nome do produto como identificador.
router.put('/:pedido/itens/substituir/:nome_antigo', authenticateToken, authorizeRole(['Gerente', 'Vendedor', 'Caixa']), async (req, res) => {
    const { pedido, nome_antigo } = req.params;
    const { nome_novo, quantidade_nova, largura, altura} = req.body;

    // Garante que o corpo da requisição contenha os dados do novo item.
    if (!nome_novo || quantidade_nova === undefined) {
        return res.status(400).json({ message: 'A requisição deve conter o nome e a quantidade do novo produto.' });
    }
    ...
    try {
        ...
        // 1. Busca e bloqueia o pedido para garantir consistência.
        const [vendaRows] = await connection.query(
            'SELECT status_pedido FROM vendas WHERE pedido = ? FOR UPDATE',
            [pedido]
        );
        ...
        // 2. Busca os dados do produto antigo a ser substituído pelo nome.
        const [produtoAntigoRows] = await connection.query(
            'SELECT codigo_barras, tipo_produto FROM produtos WHERE nome = ?',
            [nome_antigo]
        );
        const produtoAntigo = produtoAntigoRows[0];

        if (!produtoAntigo) {
            await connection.rollback();
            return res.status(404).json({ message: `Produto '${nome_antigo}' não encontrado.` });
        }

        // 3. Busca os dados do item antigo no pedido usando o código de barras.
        const [itemAntigoRows] = await connection.query(
            'SELECT quantidade FROM itens_venda WHERE pedido = ? AND codigo_barras = ? FOR UPDATE',
            [pedido, produtoAntigo.codigo_barras]
        );
        const itemAntigo = itemAntigoRows[0];
        
        if (!itemAntigo) {
            await connection.rollback();
            return res.status(404).json({ message: 'Item do pedido a ser substituído não encontrado.' });
        }

        // 4. Devolve a quantidade do item antigo ao estoque (se aplicável).
        if (produtoAntigo.tipo_produto === 'padrao') {
            await connection.query(
                'UPDATE produtos SET quantidade = quantidade + ? WHERE codigo_barras = ?',
                [itemAntigo.quantidade, produtoAntigo.codigo_barras]
            );
        }

        // 5. Deleta o item antigo do pedido.
        await connection.query('DELETE FROM itens_venda WHERE pedido = ? AND codigo_barras = ?', [pedido, produtoAntigo.codigo_barras]);

        // 6. Busca os dados do novo produto a ser inserido.
        const [produtoNovoRows] = await connection.query(
            'SELECT codigo_barras, nome, preco_venda, quantidade, tipo_produto FROM produtos WHERE nome = ?',
            [nome_novo]
        );
        const produtoNovo = produtoNovoRows[0];

        if (!produtoNovo) {
            await connection.rollback();
            throw new Error(`Produto '${nome_novo}' não encontrado.`);
        }
        
        // 7. Calcula o subtotal e atualiza o estoque do novo produto.
        let subtotalNovoItem = 0;
        if (produtoNovo.tipo_produto === 'padrao') {
            if (produtoNovo.quantidade < quantidade_nova) {
                await connection.rollback();
                throw new Error(`Estoque insuficiente para o produto ${produtoNovo.nome}. Disponível: ${produtoNovo.quantidade}`);
            }
            subtotalNovoItem = produtoNovo.preco_venda * quantidade_nova;
            await connection.query(
                'UPDATE produtos SET quantidade = quantidade - ? WHERE codigo_barras = ?',
                [quantidade_nova, produtoNovo.codigo_barras]
            );
        } else if (produtoNovo.tipo_produto === 'personalizado') {
            if (!largura || !altura) {
                await connection.rollback();
                throw new Error(`Largura e altura são obrigatórios para o produto personalizado '${produtoNovo.nome}'.`);
            }
            subtotalNovoItem = produtoNovo.preco_venda * largura * altura;
        }

        // 8. Insere o novo item na tabela itens_venda.
        await connection.query(
            'INSERT INTO itens_venda (pedido, codigo_barras, quantidade, preco_unitario, subtotal, largura, altura) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [pedido, produtoNovo.codigo_barras, quantidade_nova, produtoNovo.preco_venda, subtotalNovoItem, largura, altura]
        );

        // 9. Recalcula o valor total do pedido.
        const [totalRows] = await connection.query('SELECT SUM(subtotal) AS novo_valor_total FROM itens_venda WHERE pedido = ?', [pedido]);
        const novoValorTotal = totalRows[0].novo_valor_total || 0;
        ...
        await connection.commit();
        res.status(200).json({
            message: 'Item do pedido substituído com sucesso!',
            valor_total: novoValorTotal
        });

    } catch (error) {
        await connection.rollback();
        console.error('Erro ao substituir item do pedido:', error);
        res.status(500).json({ message: 'Erro ao substituir item do pedido.', error: error.message });
    } finally {
        connection.release();
    }
});

// DELETE /api/pedidos/:pedido/itens/:nome
// Cancela (remove) um item específico de um pedido pelo seu nome.
router.delete('/:pedido/itens/:nome', authenticateToken, authorizeRole(['Gerente', 'Vendedor', 'Caixa']), async (req, res) => {
    const { pedido, nome } = req.params;
    ...
    try {
        ...
        if (venda.status_pedido !== 'Aberto') {
            await connection.rollback();
            return res.status(400).json({ message: `Não é possível cancelar itens de um pedido com status '${venda.status_pedido}'.` });
        }

        // 2. Busca o código de barras do produto pelo nome.
        const [produtoRows] = await connection.query(
            'SELECT codigo_barras, tipo_produto FROM produtos WHERE nome = ?',
            [nome]
        );
        ...
        if (!produto) {
            await connection.rollback();
            return res.status(404).json({ message: `Produto '${nome}' não encontrado.` });
        }

        // 3. Busca os dados do item no pedido usando o código de barras.
        ...
        // 4. Devolve a quantidade ao estoque se for um produto 'padrao'.
        if (produto.tipo_produto === 'padrao') {
            await connection.query(
                'UPDATE produtos SET quantidade = quantidade + ? WHERE codigo_barras = ?',
                [item.quantidade, produto.codigo_barras]
            );
        }

        // 5. Remove o item do pedido usando o código de barras.
        await connection.query('DELETE FROM itens_venda WHERE pedido = ? AND codigo_barras = ?', [pedido, produto.codigo_barras]);

        // 6. Recalcula o valor total do pedido.
        const [totalRows] = await connection.query('SELECT SUM(subtotal) AS novo_valor_total FROM itens_venda WHERE pedido = ?', [pedido]);
        const novoValorTotal = totalRows[0].novo_valor_total || 0;
        ...
        await connection.commit();
        res.status(200).json({ message: 'Item do pedido cancelado com sucesso!' });
        
    } catch (error) {
        await connection.rollback();
        console.error('Erro ao cancelar item do pedido:', error);
        res.status(500).json({ message: 'Erro ao cancelar item do pedido.', error: error.message });
    } finally {
        connection.release();
    }
});
